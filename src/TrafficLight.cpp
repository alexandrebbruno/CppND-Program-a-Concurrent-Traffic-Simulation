...
T MessageQueue<T>::receive()
{
    ...
    auto f = [this](){return !_queue.empty();};
    _condition.wait(lock, f);
...
{
    // computing cycle duration 
    std::random_device rd;
    std::default_random_engine e{rd()};
    std::uniform_real_distribution<double> realDist(4.0, 6.0);
    double cycleDuration = realDist(e);
    ...
    while(true){
        ...
        duration = std::chrono::steady_clock::now() - init;
        ...
        if (duration.count() > cycleDuration){
            ...
            // reset init time and computing new cycle duration
            cycleDuration = realDist(e);
